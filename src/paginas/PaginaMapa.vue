...
<template>
  ...
  <div class="mv-page">
    ...
    <div class="mv-container">
      ...
      <!-- Introdução da página: explica o objetivo do mapa e a interação por hover. -->
      <div class="page-hero">
        ...
        <p class="page-hero__desc">
          ...
          Visualização geográfica dos 27 Estados-Membros com dados do Mecanismo de Recuperação
          
          e Resiliência. Passe o cursor por cima de cada país para ver os detalhes.
        </p>
      </div>
      ...
      <!-- Contentor relativo para o SVG e para os tooltips posicionados pelo rato. -->
      <div class="mv-map-shell" v-scroll-animate ref="mapShell">
        ...
        <Transition name="tip">
          ...
          <!-- Tooltip dos países vizinhos que não pertencem à UE. -->
          <div
            v-if="hoveredCtxId && hoverCtxData"
            class="mv-ctx-tip"
            :style="tipPos"
            role="tooltip"
          >
        ...
        <Transition name="tip">
          ...
          <!-- Tooltip detalhado dos Estados-Membros da UE. -->
          <div
            v-if="hoveredId && hoverData"
            class="mv-tip"
            :style="tipPos"
            role="tooltip"
          >
            ...
            <div class="mv-tip__head">
              ...
              <div>
                ...
                <p class="mv-tip__sub">{{ activeMetricLabel }}: <strong>{{ formatVal(hoverData[activeMetric]) }}</strong></p>
              </div>
            </div>
...
// Tamanho fixo do viewBox do SVG.
const W = 960, H = 680
// Recurso reativo que contém os indicadores por país.
...
// Nomes e bandeiras usados nos tooltips dos países de contexto.
const CONTEXT_NAMES = computed(() => resource.value.contextNames ?? {})
// Metadados do país expostos como computed para reagir a atualizações da API.
...
function setMetric(key) { activeMetric.value = key }
...
function getName(id) { return COUNTRY_META.value[id]?.name || id }
// Estado de carregamento do SVG.
const ready       = ref(false)
// Caminhos SVG dos Estados-Membros.
const euPaths     = ref([])
...
// Estado de hover para países de contexto.
const hoveredCtxId = ref(null)
// Coordenadas do rato dentro do contentor do mapa.
const mouseX       = ref(0)
const mouseY       = ref(0)
// Dados do tooltip ativo para país UE.
const hoverData    = computed(() => hoveredId.value ? COUNTRY_META.value[hoveredId.value] : null)
// Dados do tooltip ativo para país de contexto.
const hoverCtxData = computed(() => hoveredCtxId.value ? CONTEXT_NAMES.value[hoveredCtxId.value] : null)
function enter(e, id) {
  // Ao entrar num país da UE, fecha qualquer tooltip de contexto.
  hoveredId.value    = id
  hoveredCtxId.value = null
  updateMouse(e)
}
function enterCtx(e, id) {
  // Ao entrar num país de contexto, fecha qualquer tooltip de Estado-Membro.
  hoveredCtxId.value = id
  hoveredId.value    = null
  updateMouse(e)
}
function move(e)  { updateMouse(e) }
function leave()  { hoveredId.value = null }
function leaveCtx() { hoveredCtxId.value = null }
function updateMouse(e) {
  // Guarda a posição relativa ao mapa para o tooltip não depender da janela inteira.
...
const tipPos = computed(() => {
  // Ajusta o tooltip para não sair fora dos limites do mapa.
  if (!mapShell.value) return {}
...
.mv-page      { background: var(--color-bg-page); min-height: 100vh; padding-bottom: 60px; }

.mv-container { max-width: 1520px; margin: 0 auto; padding: 0 var(--container-padding); }
...
.mv-tabs { display: flex; gap: 6px; flex-wrap: wrap; }
.mv-tab {
...
.mv-tab:hover { border-color: rgba(29,69,135,.35); color: var(--color-brand-blue); }
.mv-tab--active {
  
  background: var(--color-brand-blue); border-color: var(--color-brand-blue);
  
  color: white; box-shadow: 0 2px 6px rgba(29,69,135,.25);
}
...
.mv-legend__track { display: flex; border-radius: 3px; overflow: hidden; }

.mv-legend__chip  { width: 28px; height: 12px; }

.mv-legend__bound { font-weight: 500; color: var(--color-text-primary); }

.mv-legend__hint  { color: var(--color-text-secondary); font-size: 11px; margin-left: 4px; }
/* Moldura do SVG e zona de posicionamento dos tooltips. */
.mv-map-shell {
  position: relative;
  margin: 0 32px;
...
.mv-loading {
  ...
  padding: 100px 0; color: var(--color-text-secondary); font-size: var(--text-base);
}
.mv-loading__spin {
...
.mv-tip__head {
  ...
  background: linear-gradient(135deg, rgba(29,69,135,.08), rgba(29,69,135,.03));
  
  border-bottom: 1px solid var(--color-border);
}

.mv-tip__flag { font-size: 22px; line-height: 1; flex-shrink: 0; }

.mv-tip__name { font-size: var(--text-base); font-weight: 600; color: var(--color-text-primary); line-height: 1.2; }

.mv-tip__sub  { font-size: var(--text-sm); color: var(--color-text-secondary); margin-top: 2px; }

.mv-tip__sub strong { color: var(--color-brand-blue); }


.mv-tip__body { padding: 10px 12px; display: flex; flex-direction: column; gap: 6px; }
.mv-tip__row {
...
.mv-tip__row strong { color: var(--color-text-primary); font-weight: 500; }

.mv-tip__row--prog { flex-direction: column; align-items: stretch; gap: 5px; }


.mv-tip__bar-wrap { display: flex; align-items: center; gap: 8px; }

.mv-tip__bar { flex: 1; height: 5px; background: rgba(29,69,135,.12); border-radius: 3px; overflow: hidden; }

.mv-tip__fill { height: 100%; background: var(--color-brand-blue); border-radius: 3px; transition: width .3s; }

.mv-tip__pct  { font-size: var(--text-sm); font-weight: 600; color: var(--color-brand-blue); min-width: 30px; text-align: right; }


.mv-tip__divider { height: 1px; background: var(--color-border); margin: 2px 0; }

.mv-tip__green { color: var(--color-green) !important; }


.tip-enter-active { transition: opacity .15s, transform .15s cubic-bezier(.34,1.56,.64,1); }

.tip-leave-active { transition: opacity .1s; }

.tip-enter-from   { opacity: 0; transform: scale(.94); }

.tip-leave-to     { opacity: 0; }
.mv-stats {
  display: grid; grid-template-columns: repeat(5,1fr);
  gap: 8px; margin: 12px 32px 0;
}
.mv-stat {
...
.mv-stat__val { font-size: 20px; font-weight: 600; color: var(--color-brand-blue); letter-spacing: -.4px; }

.mv-stat__lbl { font-size: var(--text-sm); color: var(--color-text-secondary); }
.mv-ctx-tip {
  position: absolute;
  z-index: 50;
  ...
  box-shadow: 0 6px 20px rgba(0,0,0,.11), 0 2px 6px rgba(0,0,0,.06);
  pointer-events: none;
  white-space: nowrap;
}

.mv-ctx-tip__flag { font-size: 20px; line-height: 1; flex-shrink: 0; }

.mv-ctx-tip__text { display: flex; flex-direction: column; gap: 3px; }

.mv-ctx-tip__name { font-size: var(--text-base); font-weight: 600; color: var(--color-text-primary); line-height: 1.2; }
.mv-ctx-tip__badge {
...
@media (max-width: 1100px) { .mv-stats { grid-template-columns: repeat(3,1fr); } }

@media (max-width: 640px)  {
  
  .mv-stats { grid-template-columns: repeat(2,1fr); }
  
  .mv-map-shell, .mv-controls, .mv-stats { margin-left: 0; margin-right: 0; }
}
</style>

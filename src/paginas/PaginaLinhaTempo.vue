    ...
    <div class="lt-container">
      ...
      <div class="page-hero">
        ...
        <p class="page-hero__desc">
          ...
          Os marcos institucionais do Mecanismo de Recuperação e Resiliência — da proposta
          
          legislativa de 2020 ao prazo final de execução em agosto de 2026.
        </p>
      </div>
        ...
        <TransitionGroup name="lt-event-anim" tag="div" class="lt-events-wrap">
          ...
          >
            ...
            <article class="lt-card" :class="{ 'lt-card--open': openEvent === event.id }">
              ...
              <div class="lt-card__head" @click="toggle(event.id)" role="button" :tabindex="0" @keydown.enter="toggle(event.id)">
              ...
              <Transition name="lt-expand">
                ...
                <div v-if="openEvent === event.id" class="lt-card__body">
...
import TimelineControls from '@/componentes/linha-tempo/ControlosLinhaTempo.vue'
import TimelineStatsGrid from '@/componentes/linha-tempo/GrelhaEstatisticasLinhaTempo.vue'
...
const currentDateLabel = computed(() => `${TODAY.getDate()} ${SHORT_MONTHS.value[TODAY.getMonth()]} ${TODAY.getFullYear()}`)
...
const filteredEvents = computed(() =>
  activeCategory.value === 'todos'
    ? EVENTS.value
    : EVENTS.value.filter((event) => event.type === activeCategory.value)
)
...
function measureTimeline() {
  // Mede posições reais no ecrã para a linha acabar na bolinha final, não no fim do contentor.
...
watch([filteredEvents, openEvent], () => {
...
.lt-page      { background: var(--color-bg-page); min-height: 100vh; padding-bottom: 80px; }

.lt-container { max-width: 1520px; margin: 0 auto; padding: 0 var(--container-padding); }
...
.lt-events-wrap { position: relative; z-index: 2; }
.lt-event {
  position: relative;
...
.lt-event--left  .lt-event__connector { left: calc(50% + 7px); right: calc(50% - 28px + 100%); width: 20px; }

.lt-event--right .lt-event__connector { right: calc(50% + 7px); left: calc(50% + 7px); width: 20px; }
.lt-card {
  width: 100%;
...
.lt-card--open {
  ...
  box-shadow: 0 6px 24px rgba(29,69,135,.1);
  
  border-color: rgba(29,69,135,.22);
}
.lt-card__head {
...
.lt-card__date { font-size: var(--text-sm); color: var(--color-text-secondary); }
.lt-card__title {
...
.lt-card--open .lt-card__chevron { color: var(--color-brand-blue); }
.lt-card__body {
...
.lt-card__impact, .lt-card__countries {
  
  background: rgba(29,69,135,.04);
...
.lt-card__tag-label {
  ...
  display: block; font-size: 10px; font-weight: 700; color: var(--color-brand-blue);
  
  text-transform: uppercase; letter-spacing: .5px; margin-bottom: 4px;
}

.lt-card__impact p, .lt-card__countries p {
  
  font-size: var(--text-sm); color: var(--color-text-secondary); line-height: 1.5; margin: 0;
}


.lt-expand-enter-active { transition: all .3s cubic-bezier(0.22,1,0.36,1); }

.lt-expand-leave-active { transition: all .2s ease; }

.lt-expand-enter-from, .lt-expand-leave-to { opacity: 0; transform: translateY(-8px); }


.lt-event-anim-enter-active { transition: all .4s cubic-bezier(0.22,1,0.36,1); transition-delay: var(--delay, 0ms); }

.lt-event-anim-leave-active { transition: all .2s ease; position: absolute; }

.lt-event-anim-enter-from   { opacity: 0; transform: translateY(16px); }

.lt-event-anim-leave-to     { opacity: 0; transform: translateY(-8px); }

.lt-event-anim-move         { transition: transform .4s cubic-bezier(0.22,1,0.36,1); }
.lt-end {
  --lt-end-translate: -50%;
  position: relative;
...
.lt-end__dot {
  ...
  background: white; border: 3px solid #d97706;
  z-index: 3;
...
@keyframes endPulse {
  ...
  0%, 100% { box-shadow: 0 0 0 0 rgba(217,119,6,.18); transform: translateX(var(--lt-end-translate)) scale(1); }
  
  50%      { box-shadow: 0 0 0 5px rgba(217,119,6,.0); transform: translateX(var(--lt-end-translate)) scale(1.04); }
}
@keyframes endRingPulse {
  
  0%, 100% { transform: translateX(var(--lt-end-translate)) scale(1); opacity: 0.28; }
  
  50%      { transform: translateX(var(--lt-end-translate)) scale(1.14); opacity: 0.08; }
}
.lt-end__card {
  width: calc(50% - 28px);
...
.lt-end__date  { font-size: 18px; font-weight: 700; color: var(--color-text-primary); margin-bottom: 8px; }

.lt-end__desc  { font-size: var(--text-sm); color: var(--color-text-secondary); line-height: 1.55; margin: 0; }


@media (max-width: 900px) {
  .lt-event--left, .lt-event--right {
  ...
  .lt-event__dot { left: 7px; transform: none; }
  
  .lt-event__dot:hover, .lt-event--open .lt-event__dot { transform: scale(1.35); }
  
  .lt-event__connector { display: none; }
  
  .lt-spine, .lt-spine__progress { left: 14px; transform: none; }
  
  .lt-now { left: 14px; transform: translateY(-50%); }
  
  .lt-now__label { left: 20px; right: auto; }
  
  .lt-end { --lt-end-translate: 0; justify-content: flex-start; padding-left: 32px; }
  
  .lt-end__dot { left: 5px; }
  
  .lt-end__ring { left: -3px; }
  
  .lt-end__card { width: 100%; margin-left: 0; }
}

@media (max-width: 640px) {
  
  .lt-body { margin-left: 0; margin-right: 0; }
  
  .lt-container { padding: 0 20px; }
}
</style>

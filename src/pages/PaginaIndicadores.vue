...
<template>
  <div class="ind-page">
    <div class="ind-container">
      ...
      <div class="page-hero">
        ...
        <p class="page-hero__desc">
          A Comissão Europeia definiu 14 indicators de reporte obrigatório e semestral por todos os Estados Membros,
          cobrindo áreas como energia, digitalização, educação e saúde.
        </p>
      </div>
      <!-- Exportação global de todos os indicators. -->
      <div class="ind-export-row" v-scroll-animate="'fade'">
        <AppExportMenu
          filename="indicators-comuns"
      ...
      <!-- Grelha de botões: permite escolher qual dos 14 indicators fica em detalhe. -->
      <div class="ind-selectors" role="group" aria-label="Selecionar indicador">
        ...
        >
          <div class="ind-sel__icon" aria-hidden="true">
            <IconePublico :src="`/icons/${IND_ICONS[ind.iconType] || 'target'}.svg`" :size="16" />
          </div>
      ...
      <div class="ind-detail" v-if="selected">
        <div class="ind-detail__header">
          <div class="ind-detail__header-left">
            ...
            <button
              class="ind-detail__info-btn"
              type="button"
              @click="openPopup"
              :aria-label="`Mais informação: ${selected.name}`"
            >
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                <circle cx="8" cy="8" r="6.5" stroke="currentColor" stroke-width="1.2" />
                <path d="M8 7v5M8 5.5v.01" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" />
              </svg>
            </button>
          </div>
        ...
        <div class="ind-chart-wrap">
          <IndicatorBarChart v-if="selected" :rows="selected.chartData" :unit="selected.unit" />
        </div>
    ...
    <Transition name="ind-modal">
      <div
        v-if="showPopup"
        class="ind-popup-overlay"
        ref="popupOverlayRef"
        role="dialog"
        aria-modal="true"
        :aria-labelledby="selected ? `ind-popup-title-${selected.num}` : undefined"
        tabindex="-1"
      ...
      >
        <div class="ind-popup" v-if="selected">
          ...
          <div class="ind-popup__top">
            <div class="ind-popup__badge">Indicador {{ selected.num }}</div>
            <span class="ind-popup__unit-label">{{ selected.unit }}</span>
          </div>

          <div class="ind-popup__title-row">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true" class="ind-popup__icon">
              <circle cx="10" cy="10" r="8.5" stroke="currentColor" stroke-width="1.3" />
              <path d="M10 9v6M10 7v.01" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" />
            </svg>
            <h2 :id="`ind-popup-title-${selected.num}`" class="ind-popup__title">{{ selected.name }}</h2>
          </div>

          <div class="ind-popup__section-head">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true" class="ind-popup__icon">
              <rect x="2" y="2" width="12" height="12" rx="1" stroke="currentColor" stroke-width="1.2" />
              <path d="M5 5h6M5 7.5h6M5 10h4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" /></svg
            ><span>Descrição</span>
          </div>
          ...
          <div class="ind-popup__section-head">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true" class="ind-popup__icon">
              <rect x="2" y="2" width="12" height="12" rx="1" stroke="currentColor" stroke-width="1.2" />
              <path d="M5 5h6M5 7.5h4M5 10h5" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" /></svg
            ><span>Metodologia de Cálculo</span>
          </div>
          ...
          <div class="ind-popup__info-row">
            <div class="ind-popup__info-card">
              <div class="ind-popup__info-card-head">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true" class="ind-popup__icon">
                  <rect x="1" y="2" width="12" height="10" rx="1" stroke="currentColor" stroke-width="1.1" />
                  <path
                    d="M3 5h8M3 7.5h6M3 10h4"
                    stroke="currentColor"
                    stroke-width="1.1"
                    stroke-linecap="round"
                  /></svg
                ><span class="ind-popup__info-label">Fonte de Dados</span>
              </div>
          ...
          <div class="ind-popup__section-head">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true" class="ind-popup__icon">
              <path
                d="M3 8h10M10 5l3 3-3 3M6 5L3 8l3 3"
                stroke="currentColor"
                stroke-width="1.2"
                stroke-linecap="round"
          ...
          <div class="ind-popup__tags">
            <span v-for="r in selected.related" :key="r" class="ind-popup__tag">{{ r }}</span>
          </div>

          <div class="ind-popup__note">
            <p>
              Para mais informação sobre a metodologia e definições oficiais, consulte a
              <a
                href="https://ec.europa.eu/economy_finance/recovery-and-resilience-scoreboard/"
                target="_blank"
                rel="noopener"
                class="ind-popup__note-link"
                >documentação da Comissão Europeia</a
              >.
            </p>
          </div>
        </div>
...
// Store principal onde os indicators são expostos para a página.
import { useDashboardStore } from '@/stores/painel'
import { storeToRefs } from 'pinia'
import AppExportMenu from '@/components/app/MenuExportacaoApp.vue'
import IndicatorBarChart from '@/components/indicators/GraficoBarrasIndicadores.vue'
...
const selected = computed(() => indicators.value.find((i) => i.num === selectedNum.value) ?? null)
// Muda o indicador apresentado no gráfico, tabela e exportação.
function selectIndicador(num) {
...
function openPopup() {
  ...
  nextTick(() => {
    popupOverlayRef.value?.focus({ preventScroll: true })
  })
}
...
const allIndicatorsExportDetails = computed(() => ['Todos os indicators'])
// Contexto usado para gerar nomes de ficheiro more descritivos na exportação individual.
const selectedIndicatorExportContext = computed(() => (selected.value ? [selected.value.name] : []))
...
const selectedIndicatorExportDetails = computed(() => {
  ...
  return [`Indicador ${selected.value.num}`, `Unidade: ${selected.value.unit}`]
})
onBeforeUnmount(() => {
  document.body.style.overflow = ''
})
...
/* Grelha de seleção dos indicators. */
.ind-selectors {
  display: grid;
...
.ind-detail__info-btn:hover {
  color: var(--color-brand-blue);
}

.ind-detail__header-right {
  display: flex;
  align-items: center;
  gap: 16px;
  flex-shrink: 0;
...
.ind-popup__info-value {
  font-size: var(--text-base);
  line-height: 1.4;
  margin: 0;
}

.ind-popup__info-value--strong {
  font-weight: 500;
}
...
.ind-popup__note-link {
  color: var(--color-brand-blue);
  font-weight: 500;
  text-decoration: none;
}

.ind-popup__note-link:hover {
  text-decoration: underline;
}

.ind-modal-enter-active,
.ind-modal-leave-active {
  transition: opacity 0.2s;
}

.ind-modal-enter-from,
.ind-modal-leave-to {
  opacity: 0;
}

.ind-modal-enter-active .ind-popup,
.ind-modal-leave-active .ind-popup {
  transition: transform 0.2s;
}

.ind-modal-enter-from .ind-popup,
.ind-modal-leave-to .ind-popup {

    ...
    <div class="cp-container">
      ...
      <div class="page-hero">
        ...
        <p class="page-hero__desc">
          Compare lado a lado o desempenho de dois Estados-Membros no Mecanismo de Recuperação e Resiliência.
        </p>
      </div>
        ...
        <div class="cp-chart-card" @mouseleave="hideSvgTooltip">
            ...
            <div class="grafico-tooltip__rows">
              <div v-for="row in svgTooltip.rows" :key="`${row.label}-${row.value}`" class="grafico-tooltip__row">
                ...
                <span class="grafico-tooltip__copy">
                  <span class="grafico-tooltip__label">{{ row.label }}</span
                  ><span v-if="row.detail" class="grafico-tooltip__meta">· {{ row.detail }}</span>
                </span>
          ...
          >
            ...
            <rect
              ...
              width="12"
              height="12"
              class="cp-legend-dot cp-legend-dot--b"
              rx="2"
            />
      ...
      <div v-if="!dataA || !dataB" class="cp-empty">
        <svg width="40" height="40" viewBox="0 0 40 40" fill="none" aria-hidden="true">
          ...
          <path
            d="M12 20h16M18 14l6 6-6 6"
            class="cp-empty__stroke"
            stroke-width="1.5"
...
import CompareCountriesFilterBar from '@/components/compare-countries/BarraFiltrosCompararPaises.vue'
import CompareCountriesSummaryCards from '@/components/compare-countries/CartoesResumoCompararPaises.vue'
import CompareCountriesTable from '@/components/compare-countries/TabelaCompararPaises.vue'
...
// Hook, função reutilizável, usado para adaptar os tooltips SVG, caixas de detalhe desenhadas sobre os gráficos, ao tema ativo.
import { usarModoEscuro } from '@/composables/usarModoEscuro'
// Paleta comum usada pelas caixas de detalhe dos gráficos.
import { getChartTooltipColors } from '@/utils/tooltipsGraficos'
import { ordenarPaisesPorNome } from '@/utils/paises.mjs'
// Recurso ativo da página.
const resource = usarRecursoApi('compare')
// Estado global do tema atual.
const { isDark } = usarModoEscuro()
// Cores comuns das caixas de detalhe, sincronizadas com o tema claro/escuro.
...
const countryOptions = computed(() =>
  ordenarPaisesPorNome(
    ALL.value.map((country) => ({
      value: country.value,
      label: country.name,
      flag: country.flag,
    }))
...
// Estado da caixa de detalhe usada nos gráficos SVG, desenhados como vetores, da comparação.
...
// Estilo inline que posiciona e pinta a caixa dos gráficos SVG, os desenhos vetoriais, com as mesmas cores dos gráficos Chart.js.
...
function setSvgTooltipPosition(event, owner, title, rows) {
  // Procura o cartão onde o gráfico SVG, desenhado como vetor, está colocado.
  const card = event.currentTarget.closest('.cp-chart-card')
  // Se por algum motivo não existir cartão, não mostra a caixa de detalhe.
  if (!card) return
  ...
  // Limita a posição horizontal para a caixa de detalhe não sair fora do cartão.
  ...
  // Atualiza todos os dados usados pelo template, a parte visual da caixa de detalhe.
  svgTooltip.value = {
    visible: true,
    owner,
    title,
...
function showRadarTooltip(event, owner, countryName, values, series) {
  ...
  setSvgTooltipPosition(
    event,
    owner,
    countryName,
    rAxes.value.map((axis, index) => ({
      label: axis.label,
...
function hideSvgTooltip() {
  // Oculta a caixa de detalhe quando o rato sai do cartão do gráfico.
  svgTooltip.value.visible = false
}
// Converte um valor financeiro para coordenada vertical no SVG, o desenho vetorial do gráfico.
function bY(v) {
  return BT + BH - (Math.min(v, BY_MAX.value) / BY_MAX.value) * BH
}
...
function rPt(r, i) {
  // Calcula um ponto polar do radar em coordenadas SVG, as posições dentro do desenho vetorial.
...
function rData(values) {
  // Transforma os valores normalizados do país numa polygon SVG, a forma vetorial do radar.
  return values
    .map((v, i) => {
      const p = rPt(v, i)
      return `${p.x},${p.y}`
...
const tableRows = computed(() => {
  ...
  return (resource.value.tableMetrics ?? []).map((metric) => {
    ...
    const diff =
      precision === 0 ? Math.round(a[metric.key] - b[metric.key]) : +(a[metric.key] - b[metric.key]).toFixed(precision)
    return { label: metric.label, a: a[metric.key], b: b[metric.key], diff }
  })
})
// Contexto usado no nome e notificação da exportação.
const comparisonExportContext = computed(() => (dataA.value && dataB.value ? [dataA.value.name, dataB.value.name] : []))
...
.cp-radar-shape--a {
  fill: rgba(29, 69, 135, 0.1);
  stroke: #1d4587;
}

.cp-radar-shape--b {
  fill: rgba(0, 142, 62, 0.15);
  stroke: #008e3e;
}
.cp-empty {
  display: flex;

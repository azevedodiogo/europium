    ...
    <div class="gl-container">
      ...
      <div class="page-hero">
        ...
        <p class="page-hero__desc">
          Definições dos principais termos técnicos relativos ao Mecanismo de Recuperação e Resiliência e aos Planos de
          Recuperação e Resiliência dos Estados-Membros.
        </p>
      </div>
      ...
      <div class="gl-meta">
        ...
        <button v-if="query || activeCategory !== 'Todos'" type="button" class="gl-meta__clear" @click="reset">
          Limpar filtros
        </button>
      </div>
      ...
      <div class="gl-alpha" aria-label="Índice alfabético">
        <a v-for="letter in availableLetters" :key="letter" :href="`#letter-${letter}`" class="gl-alpha__letter">{{
          letter
        }}</a>
      </div>
      ...
      <div class="gl-body" v-scroll-animate>
          ...
          <div class="gl-cards">
            ...
            >
              ...
              >
                ...
                <IconChevronDown
                  :size="16"
                  class="gl-card__chevron"
                  :style="{ transform: openTerm === term.term ? 'rotate(180deg)' : 'rotate(0)' }"
                />
              </button>
        ...
        <div v-if="filtered.length === 0" class="gl-empty">
          <svg width="56" height="56" viewBox="0 0 56 56" fill="none" aria-hidden="true">
            <path
              d="M10 6h24l10 10v30a3 3 0 01-3 3H13a3 3 0 01-3-3V9a3 3 0 013-3z"
              class="gl-empty__stroke"
              stroke-width="2"
          ...
          <p>
            Nenhum termo encontrado para <strong>«{{ query }}»</strong>
          </p>
...
// Componente separado para manter a zona de filtros more simples de ler.
import GlossaryControls from '@/components/glossary/ControlosGlossario.vue'
...
const CATEGORIES = computed(() => [
  'Todos',
  ...CATEGORY_ORDER.value.filter((category) => TERMS.value.some((term) => term.category === category)),
])
...
const availableLetters = computed(() => groupedFiltered.value.map((g) => g.letter))
</script>
...
.gl-card--open {
  box-shadow: 0 2px 12px rgba(29, 69, 135, 0.06);
}
.gl-card__head {
  width: 100%;
  display: flex;
...
.gl-card--open .gl-card__head {
  background: rgba(29, 69, 135, 0.03);
}
...
[data-theme='dark'] .gl-card__cat[data-cat='indicators'] {
...
.gl-card__related-label {
  font-size: var(--text-sm);
  color: var(--color-text-secondary);
  white-space: nowrap;
}

.gl-card__related-tags {
  display: flex;
  gap: 6px;
  flex-wrap: wrap;
}
...
.gl-card__rel-tag:hover {
  background: var(--color-brand-blue-10);
  border-color: rgba(29, 69, 135, 0.3);
}
.gl-empty {
  display: flex;
  flex-direction: column;

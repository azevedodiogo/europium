...
<template>
  ...
  <div class="des-tabpanel" role="tabpanel">
    
    <!-- Cartão superior com a evolução acumulada dos desembolsos. -->
          ...
          <div v-for="(item, index) in timeline" :key="item.id" class="des-tl-item">
            ...
            <div class="des-tl-body">
              ...
              <div class="des-tl-top">
                ...
                <span v-if="showCountryColumn" class="des-tl-country">{{ item.countryFlag }} {{ item.countryLabel }}</span>
              </div>
            ...
            <thead>
              <tr>
                ...
                <th
                  ...
                  :aria-sort="paymentSortKey === 'dateValue' ? (paymentSortDir === 'asc' ? 'ascending' : 'descending') : undefined"
                  @click="$emit('sort', 'dateValue')"
                >
                  Data
                  
                  <span v-if="paymentSortKey === 'dateValue'" aria-hidden="true">{{ paymentSortDir === 'asc' ? ' ↑' : ' ↓' }}</span>
                </th>
                ...
                <th
                  ...
                  :aria-sort="paymentSortKey === 'countryLabel' ? (paymentSortDir === 'asc' ? 'ascending' : 'descending') : undefined"
                  @click="$emit('sort', 'countryLabel')"
                >
                  País
                  
                  <span v-if="paymentSortKey === 'countryLabel'" aria-hidden="true">{{ paymentSortDir === 'asc' ? ' ↑' : ' ↓' }}</span>
                </th>
                ...
                <th
                  class="des-th des-th--sortable"
                  :aria-sort="paymentSortKey === 'parcel' ? (paymentSortDir === 'asc' ? 'ascending' : 'descending') : undefined"
                  @click="$emit('sort', 'parcel')"
                >
                  Parcela
                  
                  <span v-if="paymentSortKey === 'parcel'" aria-hidden="true">{{ paymentSortDir === 'asc' ? ' ↑' : ' ↓' }}</span>
                </th>
                ...
                <th
                  ...
                  :aria-sort="paymentSortKey === 'amountValue' ? (paymentSortDir === 'asc' ? 'ascending' : 'descending') : undefined"
                  @click="$emit('sort', 'amountValue')"
                >
                  Montante
                  
                  <span v-if="paymentSortKey === 'amountValue'" aria-hidden="true">{{ paymentSortDir === 'asc' ? ' ↑' : ' ↓' }}</span>
                </th>
                ...
                <th
                  class="des-th des-th--sortable"
                  :aria-sort="paymentSortKey === 'typeSortValue' ? (paymentSortDir === 'asc' ? 'ascending' : 'descending') : undefined"
                  @click="$emit('sort', 'typeSortValue')"
                >
                  Tipo
                  
                  <span v-if="paymentSortKey === 'typeSortValue'" aria-hidden="true">{{ paymentSortDir === 'asc' ? ' ↑' : ' ↓' }}</span>
                </th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr v-for="row in sortedPayments" :key="row.id" class="des-tr">
                ...
                <td v-if="showCountryColumn" class="des-td des-td--country">{{ row.countryFlag }} {{ row.countryLabel }}</td>
                
                <td class="des-td des-td--parcel">{{ row.parcel }}</td>
                
                <td class="des-td des-td--amount">{{ row.amount }}</td>
                
                <td class="des-td"><span class="des-badge" :data-type="row.type">{{ row.typeLabel }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
...
import DisbursementsLineChart from '@/componentes/desembolsos/GraficoLinhaDesembolsos.vue'
...
.des-tl-country {
  ...
  background: rgba(234, 239, 245, .75);
  white-space: nowrap;
}
.des-tl-date {
  font-size: var(--text-sm);
...
.des-badge[data-type="pre"] {
  ...
  background: rgba(0, 142, 62, .1);
  
  border-color: rgba(0, 142, 62, .2);
  color: #008e3e;
}
.des-badge[data-type="subvencao"] {
  
  background: rgba(29, 69, 135, .1);
...
.des-badge[data-type="emprestimo"] {
  ...
  background: rgba(216, 109, 55, .1);
  
  border-color: rgba(216, 109, 55, .2);
  color: #d86d38;
}
...
[data-theme="dark"] .des-badge[data-type="emprestimo"] {
  ...
  background: rgba(241, 177, 91, .16);
  
  border-color: rgba(241, 177, 91, .24);

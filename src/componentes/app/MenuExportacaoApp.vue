...
<template>
  <!-- Contentor relativo para ancorar o dropdown ao botão. -->
  <div class="exp" ref="rootEl">
    ...
    <Transition name="exp-drop">
      ...
      <div v-if="isOpen" class="exp__menu" role="menu">
        <!-- Cria uma opção por cada formato suportado. -->
        <button v-for="fmt in formats" :key="fmt.value" type="button" class="exp__item" @click="handleExport(fmt.value)">
          {{ fmt.label }}
        </button>
      </div>
    </Transition>
...
// Props que definem o nome do ficheiro, o contexto e se existe algo para exportar.
...
const formats = [{ value: 'json', label: 'JSON' }, { value: 'csv', label: 'CSV' }]
...
function flattenRow(value, prefix = '', output = {}) {
  if (Array.isArray(value)) {
    output[prefix || 'valor'] = value.map(item => isPlainObject(item) || Array.isArray(item) ? JSON.stringify(item) : item).join(' | ')
    return output
  }
...
<style scoped>
/* O contentor precisa de ser relativo para o dropdown ficar alinhado ao botão. */
.exp { position: relative; display: inline-block; }

/* Estilo base do botão de exportação. */
.exp__trigger {
  display: inline-flex; align-items: center; gap: 6px; height: 32px; padding: 0 12px;
  background: var(--color-bg-white); border: 1px solid var(--color-border); border-radius: var(--radius-sm);
  box-shadow: 0 1px 2px rgba(0,0,0,.05); font-family: var(--font-family); font-size: var(--text-base);
  font-weight: 500; color: var(--color-text-primary); cursor: pointer; white-space: nowrap; transition: background .15s, border-color .15s, box-shadow .15s;
}

/* Pequeno realce quando o cursor passa por cima. */
.exp__trigger:hover { background: var(--color-progress-bg); }

/* Estado visual do botão quando o menu está aberto. */
.exp__trigger--open {
  border-color: rgba(29,69,135,.42);
  box-shadow: 0 0 0 3px rgba(29,69,135,.08);
}

/* Painel do dropdown. */
.exp__menu {
  position: absolute; top: calc(100% + 6px); right: 0; width: 112px; background: var(--color-bg-white);
  border: 1px solid var(--color-border); border-radius: var(--radius-sm); overflow: visible;
  z-index: 9000; box-shadow: 0 2px 8px rgba(0,0,0,.08);
}

/* Cada linha clicável do menu. */
.exp__item {
  width: 100%; height: 32px; padding: 0 12px; background: transparent; border: none;
  border-bottom: 1px solid var(--color-border); font-family: var(--font-family); font-size: var(--text-sm);
...
.exp__item:hover { background: rgba(29,69,135,.05); }

/* Transição do dropdown. */
.exp-drop-enter-active, .exp-drop-leave-active { transition: opacity .12s, transform .12s; }

/* Estado inicial e final da animação. */
.exp-drop-enter-from, .exp-drop-leave-to { opacity: 0; transform: translateY(-4px); }

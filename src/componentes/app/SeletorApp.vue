...
-->
<template>
  <!-- Área relativa para o botão e o painel das opções. -->
  <div class="cbox" :style="{ width }" ref="rootEl">
    ...
    <button
      ...
      @keydown.down.prevent="openPanel('selected')"
      @keydown.up.prevent="openPanel('last')"
      @keydown.home.prevent="openPanel(0)"
      @keydown.end.prevent="openPanel('last')"
    >
    ...
    <Transition name="cbox-drop">
      <!-- Painel com a opção "Todos" e com a lista recebida por props, os dados enviados para o componente. -->
      <div v-if="isOpen" :id="listboxId" class="cbox__panel" role="listbox" @keydown="handlePanelKeydown">
...
const listboxId = `cbox-listbox-${Math.random().toString(36).slice(2)}`
...
const selectedOptionIndex = computed(() => {
  ...
  const optionIndex = props.options.findIndex((option) => option.value === props.modelValue)
...
function optionButtons() {
  return [...(rootEl.value?.querySelectorAll('.cbox__option') ?? [])]
}
...
function resolveFocusIndex(focusTarget) {
  const buttonsLength = props.options.length + 1
  if (focusTarget === 'selected') return selectedOptionIndex.value
  if (focusTarget === 'last') return buttonsLength - 1
  return typeof focusTarget === 'number' ? focusTarget : null
}
function openPanel(focusTarget = null) {
  isOpen.value = true
  const focusIndex = resolveFocusIndex(focusTarget)
...
function handleKey(e) {
  if (e.key === 'Escape' && isOpen.value) closePanel(true)
}
...
.cbox__trigger:hover,
.cbox__trigger--open {
  border-color: rgba(29, 69, 135, 0.4);
}

.cbox__trigger--open {
  box-shadow: 0 0 0 3px rgba(29, 69, 135, 0.07);
}
...
.cbox-drop-enter-active,
.cbox-drop-leave-active {
  transition:
    opacity 0.14s,
    transform 0.14s;
}

.cbox-drop-enter-from,
.cbox-drop-leave-to {
  opacity: 0;

      ...
      >
        ...
        <div class="kpi__value-row">
          ...
          <span
            v-if="kpi.badge"
            class="kpi__badge"
            :class="`kpi__badge--${kpi.badge.color}`"
          >{{ kpi.badge.text }}</span>
        </div>
...
.kpi__card:hover { box-shadow: var(--shadow-md); }
...
.kpi__badge { font-size: var(--text-sm); font-weight: 500; }

.kpi__badge--green { color: var(--color-green); }
.kpi__description {
  font-size: var(--text-sm);
  color: var(--color-text-secondary);
  line-height: 1.33;
...
@media (max-width: 1024px) {
  ...
  .kpi__grid { grid-template-columns: repeat(2, 1fr); padding: 0 40px; }
}

@media (max-width: 640px) {
  
  .kpi__grid { grid-template-columns: 1fr; padding: 0 20px; }
}
</style>

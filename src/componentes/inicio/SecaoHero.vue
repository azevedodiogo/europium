  ...
  <section class="hero" aria-labelledby="hero-title">
    ...
    <div class="hero__container">
      ...
      <h1 id="hero-title" class="hero__title">
        Mecanismo de<br />Recuperação e Resiliência
      </h1>
      ...
      <div class="hero__actions">
        ...
        <RouterLink to="/comparar-paises" class="hero__btn hero__btn--secondary">
          Comparar Países
        </RouterLink>
      </div>
...
.hero__vignette {
  ...
  background: radial-gradient(
    ...
    transparent 35%,
    rgba(13, 38, 128, 0.65) 100%
  );
  pointer-events: none;
  z-index: 1;
}
/* Contentor central do hero. */
.hero__container {
  max-width: 1520px;
  width: 100%;
  margin: 0 auto;
...
[data-theme="dark"] .hero__btn--primary {
  ...
  background: rgba(240,247,255,.14);
  color: #e6f0ff;
  
  border: 1px solid rgba(157,192,244,.26);
  
  box-shadow: inset 0 1px 0 rgba(255,255,255,.08);
}
[data-theme="dark"] .hero__btn--primary:hover {
  
  background: rgba(240,247,255,.18);
  
  box-shadow:
    inset 0 1px 0 rgba(255,255,255,.1),
    0 8px 24px rgba(8,13,24,.18);
}
[data-theme="dark"] .hero__btn--secondary {
  
  background: rgba(119,167,234,.08);
  
  border-color: rgba(157,192,244,.18);
  color: #eef4ff;
}
[data-theme="dark"] .hero__btn--secondary:hover {
  
  background: rgba(119,167,234,.14);
}
...
.hero__scroll-text { letter-spacing: .3px; }

.hero__scroll-arrow { animation: bounce 2s ease-in-out infinite; }
@keyframes bounce {
...
@media (max-width: 900px) {
  ...
  .hero__container { padding: 60px 24px 40px; }
}
</style>
